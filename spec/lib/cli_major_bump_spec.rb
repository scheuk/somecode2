  ...
  describe "version" do
    ...
    before {
      ...
      Dir.chdir(git_dir) {
        ...
        %x[git add somefile]
        %x[git commit -m "init" .]
        %x[git #{version_object} version/1.1.32]
        %x[git #{version_object} version/1.1.33]
        %x[git #{version_object} version/1.1.34]
        %x[git #{version_object} origin/version/1.1.31]
        %x[git #{version_object} origin/version/1.1.32]
        %x[git #{version_object} origin/version/1.1.33]
        %x[git #{version_object} origin/version/1.1.34]
      }
    }
